...
class MeanReversion(Strategy):
    ...
    def next(self):
        ...
        # --- CORRECTED: Bollinger Band Entry Condition ---
        # If price crosses below the lower band, buy
        if crossover(self.lower_band, self.data.Close) and not self.position:
            self.buy(sl=sl_price, tp=tp_price)
            
        # Optional exit: Close if price touches the upper band
...
print("\nRunning optimization for Mean Reversion strategy...")
stats = bt.optimize(
    bb_period=[10, 20, 30],
    bb_std_dev=[1.5, 2.0, 2.5],
    stop_loss_pct=list(np.arange(0.02, 0.11, 0.02)),  # 2% to 10% SL
    take_profit_pct=list(np.arange(0.04, 0.21, 0.04)), # 4% to 20% TP
    maximize='Sortino Ratio'
)

# Print and Plot Results
print("\n--- Backtest Results (Optimized Mean Reversion) ---")
print(stats)
print("\n--- Optimal Parameters ---")
